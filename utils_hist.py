...
def estimate_init_pose(args, src, dst):
    ...
    # memeory may overflow when batch size is large. If that is the case, see solution below (commented)
    t_hist = hist(dst, src, 
...
# #### in case memory overflows, you may try the following setup; change the chunk size to fit your GPUs if necessary
# def estimate_init_pose(args, src, dst):
#     # sometimes memeory overflows when time duration increases
#     chunk = 50
#     transformations = []
#     assert len(src) == len(dst)
#     n = len(src)//chunk if len(src) % chunk ==0 else len(src)//chunk +1
#     for k in range(0, n):
#         transformation = estimate_init_pose_batch(args, src[k*chunk: (k+1)*chunk], dst[k*chunk: (k+1)*chunk])

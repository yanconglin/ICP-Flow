...
struct PointXYZ {
    ...
    PointXYZ(float _x, float _y, float _z, int _index) : x(_x), y(_y), z(_z), index(_index) {}
};
...
struct Params 
{
    ...
    Params() {
        ...
        max_range = 32.0;           // max_range of ground estimation area
...
public:
    ...
    std::vector<int> getNongroundIndices();

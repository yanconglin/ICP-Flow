    ...
    for (int zone_idx = 0; zone_idx < params_.num_zones; ++zone_idx) {
        ...
        for (int ring_idx = 0; ring_idx < params_.num_rings_each_zone[zone_idx]; ++ring_idx) {
            for (int sector_idx = 0; sector_idx < params_.num_sectors_each_zone[zone_idx]; ++sector_idx) {
                ...
                centers_.push_back(PointXYZ(pc_mean_(0), pc_mean_(1), pc_mean_(2), 0));
                normals_.push_back(PointXYZ(normal_(0), normal_(1), normal_(2), 0));
...
void PatchWorkpp::reflected_noise_removal(Eigen::MatrixXf &cloud_in)
{
    ...
    for (int i=0; i<cloud_in.rows(); i++)
    {
        ...
        {
            cloud_nonground_.push_back(PointXYZ(cloud_in.row(i)(0), cloud_in.row(i)(1), cloud_in.row(i)(2), int(cloud_in.row(i)(4))));
    ...
    for (int i=0; i<src.rows(); i++) {
        ...
        if ((r <= max_range) && (r > min_range)) {
            ...
            if (r < min_range_1) { // In First rings
                ...
                czm[0][ring_idx][sector_idx].emplace_back(PointXYZ(x, y, z, int(src.row(i)(4))));
                ...
                czm[1][ring_idx][sector_idx].emplace_back(PointXYZ(x, y, z, int(src.row(i)(4))));
                ...
                czm[2][ring_idx][sector_idx].emplace_back(PointXYZ(x, y, z, int(src.row(i)(4))));
            } else { // Far!
                ...
                czm[3][ring_idx][sector_idx].emplace_back(PointXYZ(x, y, z, int(src.row(i)(4))));
            }

        } else {
            cloud_nonground_.push_back(PointXYZ(x, y, z, int(src.row(i)(4))));
        }
    }
...
std::vector<int> PatchWorkpp::getGroundIndices()
{
    std::vector<int> groundIndices;
    groundIndices.reserve(cloud_ground_.size());
    for (int i = 0; i < cloud_ground_.size(); ++i)
    {
        groundIndices.push_back(cloud_ground_[i].index);
    }
    return groundIndices;
}

std::vector<int> PatchWorkpp::getNongroundIndices()
{
    std::vector<int> NongroundIndices;
    NongroundIndices.reserve(cloud_nonground_.size());
    for (int i = 0; i < cloud_nonground_.size(); ++i)
    {
        NongroundIndices.push_back(cloud_nonground_[i].index);
    }
    return NongroundIndices;
}